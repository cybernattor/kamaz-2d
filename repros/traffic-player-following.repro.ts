...
function main() {
  ...
  if (car.speed > 0.8) {
    throw new Error(`NPC did not brake behind a stopped player: speed ${car.speed.toFixed(1)}`);
  }

  console.log(`FIXED: NPC followed a stopped player without impact; closest gap ${closestGap.toFixed(1)}.`);
}
